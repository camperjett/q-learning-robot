...
from gripper import loadWorld, loadGripper, reachXY, pick, place
...
def q_learning():
    ...
    for i in range(0, 10):
        ...
        while not (x == a and y == b):
            ...
            pos = np.array(pos)[np.array(distance).argsort()]
            distance.sort()
            reward = 0
            ...
            if x == a and y == b:
                ...
                # Since q(s',a') = (q(s,a) - reward)/gamma, maximum value max_q of q(s',a') = (q(s,a) - (minimum reward))/gamma
    ...
    print(f"\nTotal number of episodes to arrange all objects : {len(it)}")
    print(f"\nAverage number of episodes to arrange an object : {len(it) / len(objects)}\n")
    f.close()
    plt.figure(4)
