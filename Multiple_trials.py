...
def q_learning(task):
    ...
    print(efficiency)
    print(f"\nAverage efficiency in task #{task + 1}: {average_efficiency}%\n")
    f.close()
...
if __name__ == "__main__":
    tasks = 0
    for tasks in range(0, 100000):
        with open("Tasks.txt", "a") as f:
            sys.stdout = f
            q_learning(tasks)
    f = open("Tasks.txt", "a")
    print(f"Average efficiency achieved in {tasks + 1} tasks: {total_efficiency/(tasks + 1)}", file=f)
    f.close()
